...
class TestGetRequests:
    ...
    def test_get_user_by_id(self):
        response_all_users = api_requests_wrapper.get_request(ApiUrls.URL, None, CommonUtility.get_custom_header(),
                                                              200)
        first_user_id = response_all_users[0]['id']
        email = response_all_users[0]['email']
        ...
        print(email)
        assert first_user_id == response_user['id'], f"got {first_user_id} but expected is {response_user['id']} "
        assert email == response_user['email'], f"got {email} but expected is {response_user['email']} "
